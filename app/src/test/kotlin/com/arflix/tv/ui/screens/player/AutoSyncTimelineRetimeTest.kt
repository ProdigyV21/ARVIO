...
import com.arflix.tv.ui.screens.player.subtitles.SubtitleSyncCue
...
import com.arflix.tv.ui.screens.player.subtitles.AutoSyncTimelineRetimeResult
import com.arflix.tv.ui.screens.player.subtitles.preferTighterFitOnDisagreement
...
class AutoSyncTimelineRetimeTest {
    ...
    fun supportedTwoToTwoReplyGapMovesOnlyTheEarlySecondStart() {
        val reference = listOf(
            cue(5_000L, 6_000L, "r0"),
            cue(10_000L, 11_000L, "r1"),
            cue(13_000L, 14_000L, "r2"),
            cue(18_000L, 19_000L, "r3"),
        )
        val target = listOf(
            cue(5_000L, 6_000L, "t0"),
            cue(10_000L, 12_000L, "t1"),
            cue(12_000L, 14_000L, "t2"),
            cue(18_000L, 19_000L, "t3"),
        )
    ...
    fun groupedReplyRepairAbstainsWithoutTwoSidedSimpleContext() {
        val reference = listOf(
            cue(10_000L, 11_000L, "r0"),
            cue(13_000L, 14_000L, "r1"),
            cue(18_000L, 19_000L, "r2"),
            cue(22_000L, 23_000L, "r3"),
        )
        val target = listOf(
            cue(10_000L, 12_000L, "t0"),
            cue(12_000L, 14_000L, "t1"),
            cue(18_000L, 19_000L, "t2"),
            cue(22_000L, 23_000L, "t3"),
        )
    ...
    fun groupedReplyRepairAbstainsFromLargeInternalStartMoves() {
        val reference = listOf(
            cue(5_000L, 6_000L, "r0"),
            cue(10_000L, 11_000L, "r1"),
            cue(13_500L, 14_500L, "r2"),
            cue(18_000L, 19_000L, "r3"),
        )
        val target = listOf(
            cue(5_000L, 6_000L, "t0"),
            cue(10_000L, 12_000L, "t1"),
            cue(12_000L, 14_500L, "t2"),
            cue(18_000L, 19_000L, "t3"),
        )
        ...
        val reference = buildList {
            ...
            base.forEachIndexed { index, cue ->
                if (index % 2 == 0) {
                    ...
                    add(cue(start, start + 900L, "sdh extra $index"))
                }
            }
    ...
    fun activityAlignmentFindsCommonFpsDrift() {
        val reference = irregularTimeline(260)
        val scale = 25.0 / 23.976
        val target = reference.map { cue -> cue((cue.startTimeMs / scale).toLong(), (cue.endTimeMs / scale).toLong(), cue.text) }
    ...
    fun externalTimelineNormalizationSortsAndDeduplicatesOnlyExactDuplicates() {
        val normalized = AutoSyncTimelineRetimer.normalizeExternalTimeline(
            listOf(
                cue(5_000L, 6_000L, "later"),
                cue(1_000L, 2_000L, "first"),
                cue(1_000L, 2_000L, "first"),
                cue(1_000L, 2_000L, "different simultaneous text"),
            ),
        )
    ...
    fun estimatedReferenceEndsAreDownweightedWithoutChangingNormalScoring() {
        val target = (0 until 40).map { index ->
            ...
            cue(start, start + 900L, "target $index")
        }
    ...
    fun retimingDoesNotCreateNewSequentialCueOverlaps() {
        val target = listOf(
            cue(10_000L, 13_000L, "a"),
            cue(13_100L, 15_500L, "b"),
            cue(16_000L, 18_500L, "c"),
            cue(19_000L, 21_500L, "d"),
            cue(22_000L, 24_500L, "e"),
            cue(25_000L, 27_500L, "f"),
        )
        val reference = listOf(
            cue(11_000L, 12_900L, "r0"),
            cue(13_500L, 14_900L, "r1"),
            cue(16_800L, 18_000L, "r2"),
            cue(19_800L, 21_000L, "r3"),
            cue(22_800L, 24_000L, "r4"),
            cue(25_800L, 27_000L, "r5"),
        )
        ...
        val target = buildList {
            reference.forEachIndexed { index, cue ->
                if (index % 10 == 0) {
                    ...
                    add(cue(cue.startTimeMs, middle, "part a"))
                    add(cue(middle + 1L, cue.endTimeMs, "part b"))
    ...
    fun activityAlignmentToleratesExtraReferenceSdhActivity() {
        ...
        val reference = buildList {
            ...
            for (index in 8 until base.lastIndex step 13) {
                ...
                add(cue(start, start + 900L, "sound effect $index"))
            }
        }.sortedBy { it.startTimeMs }
    ...
    fun localizedSkipRunIsAcceptedWhenEveryOtherConfidenceGatePasses() {
        ...
        val localizedExtras = (0 until 14).map { index ->
            ...
            cue(start, start + 700L, "local extra $index")
        }
    ...
    fun activityAlignmentRejectsUnrelatedTimeline() {
        ...
        val target = (0 until 205).map { index ->
            ...
            cue(start, start + 700L + (index % 5) * 310L, "unrelated $index")
        }
    ...
    private fun irregularTimeline(count: Int): List<SubtitleSyncCue> {
        ...
        return (0 until count).map { index ->
            ...
            cue(start, start + 900L + ((index * 313L) % 1_700L), "irregular $index")
        }
    }
    ...
    private fun repeatedCadenceTimeline(count: Int): List<SubtitleSyncCue> {
        ...
        return (0 until count).map { index ->
            ...
            cue(start, start + 1_000L + (index % 4) * 190L, "cadence $index")
        }
    }
}

// ── kotlin.test → JUnit shim ────────────────────────────────────────────────────
private fun cue(startTimeMs: Long, endTimeMs: Long = startTimeMs + 5_000L, text: String) =
    SubtitleSyncMatcher.TimedCue(startTimeMs, endTimeMs, text)

private fun SubtitleSyncMatcher.TimedCue.copy(
    startTimeMs: Long = this.startMs,
    endTimeMs: Long = this.endMs,
...
private fun <T : Any> assertNotNull(actual: T?, message: String? = null): T {
