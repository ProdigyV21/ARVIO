        ...
        if (TextUtils.equals(RAW_RESOURCE_SCHEME, uriNormalizeScheme.getScheme())) {
            ...
            if (packageName.equals(context.getPackageName())) {
                ...
                try {
                    ...
                    throw new RawResourceDataSourceException("Package in android.resource:// URI not found. Check http://g.co/dev/packagevisibility.", e5, 2005);
                }
            }
            if (path.matches("\\d+")) {
                identifier = parseResourceId(path);
            } else {
                identifier = resourcesForApplication.getIdentifier(androidx.compose.foundation.c.t(packageName, ":", path), "raw", null);
                if (identifier == 0) {
