...
public final class FloatExponentialDecaySpec implements FloatDecayAnimationSpec {
    ...
    public float getTargetValue(float initialValue, float initialVelocity) {
        ...
        return ((initialVelocity / f10) * ((float) Math.exp((((double) f10) * ((dLog / ((double) f10)) * ((double) 1000))) / ((double) 1000.0f)))) + (initialValue - (initialVelocity / f10));
    }

    @Override // androidx.compose.animation.core.FloatDecayAnimationSpec
    public float getValueFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        float f10 = this.friction;
        return ((initialVelocity / f10) * ((float) Math.exp((f10 * (playTimeNanos / 1000000)) / 1000.0f))) + (initialValue - (initialVelocity / f10));
    }
    ...
    public /* synthetic */ FloatExponentialDecaySpec(float f10, float f11, int i10, h hVar) {
