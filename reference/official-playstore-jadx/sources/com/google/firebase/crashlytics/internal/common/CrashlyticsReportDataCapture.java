...
public class CrashlyticsReportDataCapture {
    ...
    static {
        ...
        a2.t(5, map, "armeabi", 6, "armeabi-v7a");
        a2.t(9, map, "arm64-v8a", 0, "x86");
        map.put("x86_64", 1);
    ...
    private CrashlyticsReport.Session.Event.Application populateEventApplicationData(int i10, TrimmedThrowableData trimmedThrowableData, Thread thread, int i11, int i12, boolean z) {
        Boolean boolValueOf;
    ...
    private CrashlyticsReport.Session.Event.Application.Execution populateExecutionData(TrimmedThrowableData trimmedThrowableData, Thread thread, int i10, int i11, boolean z) {
        return CrashlyticsReport.Session.Event.Application.Execution.builder().setThreads(populateThreadsList(trimmedThrowableData, thread, i10, z)).setException(populateExceptionData(trimmedThrowableData, i10, i11)).setSignal(populateSignalData()).setBinaries(populateBinaryImagesList()).build();
    }
    ...
    private List<CrashlyticsReport.Session.Event.Application.Execution.Thread> populateThreadsList(TrimmedThrowableData trimmedThrowableData, Thread thread, int i10, boolean z) {
        ArrayList arrayList = new ArrayList();
    ...
    public CrashlyticsReport.Session.Event captureEventData(Throwable th, Thread thread, String str, long j10, int i10, int i11, boolean z) {
        int i12 = this.context.getResources().getConfiguration().orientation;
        return CrashlyticsReport.Session.Event.builder().setType(str).setTimestamp(j10).setApp(populateEventApplicationData(i12, TrimmedThrowableData.makeTrimmedThrowableData(th, this.stackTraceTrimmingStrategy), thread, i10, i11, z)).setDevice(populateEventDeviceData(i12)).build();
    }
